...
export default function ProductEdit({ attributes, setAttributes }) {
	...
	const inspectorControls = (
		<InspectorControls>
				...
				<PanelBody
					title={__('Layout', 'expose')}
					initialOpen={true}
				>
					<RangeControl
						label={__("Columns on small screens", 'expose')}
						max={ 6 }
						min={ 1 }
						help={__("eg. Smartphones", 'expose')}
						onChange={(value) => {setAttributes( { columnsSmall: value })}}
						value={ columnsSmall }
					/>
					<RangeControl
						label={__("Columns on medium screens", 'expose')}
						max={ 6 }
						min={ 1 }
						help={__("Tablets and smaller screens", 'expose')}
						onChange={(value) => {setAttributes( { columnsMedium: value })}}
						value={ columnsMedium }
					/>
		
					<RangeControl
						label={__("Columns on large screens", 'expose')}
						max={ 6 }
						min={ 1 }
						help={__("Desktop screens", 'expose')}
				...
				<PanelBody
					title={__('Appearance', 'expose')}
					initialOpen={true}
				>

                    <label className="components-base-control__label" htmlFor="inspector-range-control-4">{__("Style", "expose")}</label><br />
		...
		<>
			...
			<div className="components-placeholder is-large">
                <div className="components-placeholder__label">
                    ...
                    </span>{__("Product Shop", "expose")}</div>
                <div className="components-placeholder__instructions">{__("See for settings in the inspector. The result can be seen in the frontend", "expose")}</div>
            </div>
